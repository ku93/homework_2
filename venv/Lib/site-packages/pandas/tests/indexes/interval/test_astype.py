...
class TestFloatSubtype(AstypeTests):
    ...
    indexes = [
        ...
        IntervalIndex.from_arrays([-1.5, np.nan, 0.0, 0.0, 1.5], [-0.5, np.nan, 1.0, 1.0, 3.0], closed="both"),
    ]
    ...
    def test_subtype_integer_errors(self):
        ...
        dtype = IntervalDtype("uint64", "right")
        msg = re.escape(
            "Cannot convert interval[float64, right] to interval[uint64, right]; " "subtypes are incompatible"
        )
        with pytest.raises(TypeError, match=msg):
            index.astype(dtype)
...
class TestDatetimelikeSubtype(AstypeTests):
    ...
    def test_subtype_integer(self, index, subtype):
        ...
        if subtype != "int64":
            msg = r"Cannot convert interval\[(timedelta64|datetime64)\[ns.*\], .*\] " r"to interval\[uint64, .*\]"
            with pytest.raises(TypeError, match=msg):
                index.astype(dtype)
            return
