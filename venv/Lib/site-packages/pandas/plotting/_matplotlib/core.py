...
class MPLPlot(ABC):
    ...
    def _validate_subplots_kwarg(
        ...
        if kind not in supported_kinds:
            raise ValueError(
                "When subplots is an iterable, kind must be " f"one of {', '.join(supported_kinds)}. Got {kind}."
            )
        ...
        # errors are a column in the dataframe
            ...
            if isinstance(data, ABCSeries) and err_shape[0] == 2:
                ...
                if err_shape[2] != len(data):
                    raise ValueError("Asymmetrical error bars should be provided " f"with the shape (2, {len(data)})")
            elif isinstance(data, ABCDataFrame) and err.ndim == 3:
                if (err_shape[0] != nseries) or (err_shape[1] != 2) or (err_shape[2] != len(data)):
                    raise ValueError(
                        "Asymmetrical error bars should be provided " f"with the shape ({nseries}, 2, {len(data)})"
                    )
